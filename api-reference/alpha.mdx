...
Request heavier per-vault analytics only when your product needs them, rather than paying the response-size and compute cost on every detailed-vault query. Selectable analytics sections currently include max drawdown, withdrawal pause ratio, composition, utilization rate, and the full V2 detailed-vault payload where supported.
