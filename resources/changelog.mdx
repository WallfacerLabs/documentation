...
## Week 36, 2026 — Sep 1–7

**API**

- Added **borrow support for Fluid** in Beta, covering supported single-collateral, single-debt markets, with position tracking and supply, withdraw, borrow, and repay transaction flows.

- Improved **borrow transaction context** with protocol-specific action steps, including collateral and controller activation where required. Integrators can follow the returned sequence when preparing transactions.

- Improved **vault composition accuracy** across supported multi-strategy vaults. Allocation discovery now includes positions held directly by the vault, and small incidental balances no longer hide underlying Morpho market allocations.

- Fixed **Jupiter Lend pricing** on Solana, addressing missing asset prices and USD TVL that caused affected vaults to disappear from discovery and become unavailable through transaction routes.

- Added **ether.fi eBTC withdrawal-queue support on Optimism**, extending queued redemption support through the shared Transactions API.

- Improved **Kamino transaction payload generation** for deposits and withdrawals, using address lookup tables to produce more compact transactions.

---

## Week 35, 2026 — Aug 25–31

**API**

- Added full **protocol IDs** to API responses and filtering. `protocol.protocolId` distinguishes protocol products and versions, while `allowedProtocols` and `disallowedProtocols` can now target either a broad protocol name or a specific protocol ID.

- Added **Aave V4 transaction support**. Supported Aave V4 Standard Spoke opportunities can now provide instant deposit and withdrawal context and payloads through the shared Transactions API.

- Expanded **Advanced Analytics composition coverage** with persisted allocation data. `select.composition` can now return stored underlying positions for more custody-based and composite strategies, alongside existing protocol-specific composition sources.

**App**

- Added **Fixed Term to Discover**. Users can now browse active Pendle principal-token markets, filter and sort by asset, network, APY, TVL, liquidity, or maturity, and open dedicated market pages with projected yield, maturity context, and related opportunities.

- Improved **Reputation Score display** by withholding ratings when holder data is unavailable, rather than showing an incomplete score as if it were fully assessed.

---

## Week 34, 2026 — Aug 18–24

**API**

- Expanded **fixed-term market data** in Beta. `GET /beta/fixed-term/` and `GET /beta/fixed-term/detailed/{network}/{vaultId}` now return richer Pendle market, asset, liquidity, maturity, protocol, and yield context through a more consistent response model.

- Added **Flying Tulip coverage**, including analytics for Flying Tulip Lend markets and the sftUSD savings vault, plus transaction support for supported Flying Tulip Lend deposit and withdrawal flows.

- Added **Sky stUSDS coverage**, making stUSDS opportunities available through vault discovery and related vault data surfaces.

- Expanded **vault composition coverage** for Yearn V3 and restored accurate allocation discovery for additional supported protocols, including Overnight Plus. Advanced Analytics can now explain the underlying positions of more multi-strategy vaults.

- Corrected **Wildcat capacity data** so remaining deposit capacity is based on the market's actual deposit limit rather than its current liquid assets. This prevents markets with available capacity from appearing full.

- Improved **Aave borrow position health factors**. Supply-only positions with no debt now return the appropriate no-debt health state instead of an invalid or misleading value.

- Added optional **API usage alerts** for customers who want email notifications when their keys encounter repeated credit or rate-limit errors.

---

## Week 33, 2026 — Aug 11–17

**API**

- Added **fixed-term market discovery** in Beta with `GET /beta/fixed-term/`. Integrators can discover active Pendle principal-token markets with current fixed APY, maturity, TVL, and liquidity, then filter and sort the results for fixed-yield experiences.

- Expanded **borrow market filtering**. Beta borrow discovery now supports protocol filters alongside the broader vault-style network, asset, TVL, pagination, and sorting controls.

- Standardized **SVM APY values** to the same decimal format used by V2 endpoints. For example, a 5% APY is now returned as `0.05` instead of `5`. This is a breaking correction for Beta SVM clients using the previous display-percent format.

- Added **curator metadata to SVM vault responses**, helping integrators show who manages supported Solana vault strategies.

- Improved **Drift APY accuracy** by including unsettled funding in vault equity snapshots, reducing cases where short-window APY appeared flat despite active funding gains or losses.

- Added normalized **Veda fee metadata** to detailed-vault responses, giving integrators clearer performance- and management-fee information for supported Veda/BoringVault opportunities.

- Expanded **composition analytics** across more multi-strategy protocols, including Yearn V2, Notional, Mellow, Syrup, Upshift, and additional Morpho V2 vaults.

---

## Week 32, 2026 — Aug 4–10

**API**

- Added **Aave transaction support on Monad**. Supported Monad Aave opportunities can now provide deposit and withdrawal transaction context and payloads through the shared Transactions API.

- Improved numeric vault filters so explicit zero values are handled correctly, including filters such as `maxTvl=0`, instead of being treated as omitted.

- Expanded **vault composition accuracy** across several multi-strategy protocols, improving how Advanced Analytics identifies and reports underlying allocations for supported Beefy, Harvest, IPOR Fusion, Lazy Summer, Notional, Superform, Syrup, and other composite vaults.

**App**

- Improved borrow-market charts with both supply and borrow rates, clearer protocol context, and available-to-borrow liquidity.

---

## Week 31, 2026 — Jul 28–Aug 3

**API**

- Moved **Advanced Analytics into Beta**. Integrators can now request heavier vault analytics from the Beta API with `POST /beta/advanced-analytics` and `POST /beta/advanced-analytics/{network}/{vaultId}`, including max drawdown, withdrawal pause ratio, composition, utilization rate, and embedded detailed-vault data.

- Added **Euler borrow market coverage** in Beta. `GET /beta/borrow/markets/` can now return Euler markets alongside existing borrow-market sources, helping integrators build broader borrow dashboards and account-health views through the shared Beta borrow model.

- Improved **borrow position chaining**. Borrow position responses now include market-specific asset identifiers, making it easier to connect a user's supplied or borrowed asset position to the matching market detail, historical data, events, NRT data, or transaction context.

- Expanded **Morpho V2 composition analytics**. Advanced Analytics can now return allocation data for additional Morpho V2 vaults where composition data is available, helping integrators show how vault assets are deployed underneath the top-level opportunity.

**App**

- Added **borrow market detail pages**. Users can now inspect supported borrow markets in a dedicated view with market-level context instead of only seeing borrow opportunities in list views.

- Improved opportunity pages for agent and search discovery. Opportunity pages now expose clearer structured vault metadata and onchain identity information, making them easier for search engines and AI agents to interpret.

---

## Week 30, 2026 — Jul 21–27

**API**

- Added **utilization rate** to Alpha advanced analytics. Integrators can now request `select.utilizationRate` on `POST /alpha/advanced-analytics` and `POST /alpha/advanced-analytics/{network}/{vaultId}` to see how much of a vault or lending reserve is currently utilized. The field returns Morpho allocator utilization, Aave/Spark reserve utilization where available, and `null` when utilization is not supported.

- Updated Reputation Score behavior so `score.networkScore` is no longer included in the active vault score. The field remains in V2 detailed-vault responses as deprecated diagnostic data for backwards compatibility.

- Added Morpho Blue liquidation-penalty support to Beta borrow market data. Morpho borrow-market responses can now include the pair-level liquidation penalty derived from the market LLTV, helping integrators show clearer liquidation-risk context.

- Expanded Merkl reward ingestion for **Aave net-lending campaigns**, improving reward APY coverage for eligible Aave-style lending opportunities.

- Improved wrapped-asset discovery and unwrapping support so vaults that rely on wrapped or receipt-token assets can be indexed and priced more reliably.

**App**

- Added a **Show APY as avg. of** control on vault detail pages, letting users switch the displayed yield window directly from the opportunity view.

- Improved borrow-market discovery previews by sorting collateral icons by collateral TVL, so the most relevant assets appear first in compact market cards.

- Made the dashboard more resilient when borrow positions fail to load. App vault positions can still render even if the borrow-position request errors.

- Added an in-app case-studies panel so users can browse customer and partner examples without leaving the app.

---

## Week 29, 2026 — Jul 14–20

**API**

- Added near-real-time **borrow market data** in Beta. Integrators can now call `GET /beta/borrow/markets/nrt/{network}/{marketId}/{assetId}` to fetch current market-asset values such as oracle price, supply rate, borrow rate, utilization, total supplied, total borrowed, and available liquidity.

- Expanded borrow market liquidity fields. Beta borrow market responses now expose `availableLiquidity` and `availableToBorrow` where available, alongside supplied and borrowed balances, making it easier to show whether a borrow asset has usable capacity now, not just whether the market exists.

- Improved borrow market detail lookups and vault aliasing. `GET /beta/borrow/markets/{network}/{marketId}` now returns richer market metadata and asset-level data, and detailed-vault market aliases are more consistent with borrow market identifiers.

- Improved borrow market freshness. Borrow endpoints now use the most recent historical market data available, and `minTvl` filtering is calculated from aggregation asset prices so borrow-market discovery behaves more predictably across assets.

- Improved **Morpho composition** analytics. Alpha advanced analytics now includes idle assets in `select.composition` results, so allocation breakdowns can account for both deployed markets and undeployed vault assets.

- Fixed Aave withdrawal-pause event handling for advanced analytics, improving `withdrawalPauseRatio` accuracy for Aave-based vaults.

**App**

- Added **borrow positions** to the dashboard. Users can now see supplied and borrowed positions from supported borrow markets directly in their portfolio view, with borrow debt reflected in the header portfolio total.

- Added **vault composition** to opportunity pages. Vault detail pages can now show current allocations for supported vaults, giving users a clearer view of where deposited assets are deployed.

- Improved borrow and transaction messaging. Unsupported and not-eligible transaction states now have clearer copy, reducing confusion when a vault action is unavailable.

- Improved discovery result visibility by showing the filtered vault count in search/filter surfaces.

---

## Week 28, 2026 — Jul 7–13

**API**

- Added single-vault **advanced analytics** in Alpha. Integrators can now call `POST /alpha/advanced-analytics/{network}/{vaultId}` to request analytics for one vault directly, instead of filtering through the paginated list endpoint. The same `select` model applies for `maxDrawdown`, `withdrawalPauseRatio`, `composition`, and `detailedVault`.

- Expanded **Morpho composition** coverage in Alpha advanced analytics. `select.composition` now supports MetaMorpho-style vaults in addition to Morpho Vault V2 adapter-based vaults, returning market-level allocations where available.

- Added richer **Beta borrow market filtering**. `GET /beta/borrow/markets` and `GET /beta/borrow/markets/{network}` now support filters for borrow assets, collateral assets, included/excluded networks, and `minTvl`, making it easier to build focused borrow-market discovery views.

- Standardized borrow-market asset identifiers. Borrow market historical, transaction, portfolio-event, and asset-position routes now use `{assetId}` in the path, and borrow market responses expose `assetsData[].marketAssetId` for market-specific asset lookups.

- Improved Aave transaction/event handling so rebasing-interest `Burn` events are only treated as withdrawals when they belong to the user. This reduces false withdrawal entries in Aave portfolio history and return calculations.

**App**

- Added the **Borrow** page to the main app navigation.

- Improved borrow-market discovery by defaulting the page toward markets above $100k TVL, tightening filter options around the visible market set, and showing result counts after filters are applied.

- Refined borrow and discovery filter menus so long option lists scroll more cleanly without pushing the rest of the interface around.

- Updated category icons and colors across the app for clearer visual grouping of vault categories.

---

## Week 27, 2026 — Jun 30–Jul 6

**API**

- Added **Robinhood** network support across V2 network-aware endpoints. `GET /v2/networks` now includes `robinhood` / `eip155:4663`, and Robinhood vaults can be discovered through standard V2 vault filters such as `allowedNetworks=robinhood`.

- Added initial **Morpho vault coverage on Robinhood**. V2 detailed vault responses now include Robinhood Morpho opportunities with the same vault metadata, APY, TVL, capacity, transaction-readiness, and app links used on other supported networks.

- Added protocol **display names** to API responses. Protocol objects now include `displayName` where available, giving integrators user-facing labels such as `Morpho`, `Aave`, or `Ether.fi` while preserving the canonical `name` field for filtering and stable identifiers.

- Expanded withdrawal-pause analytics coverage in Alpha. `POST /alpha/advanced-analytics` can now calculate `withdrawalPauseRatio` for more protocols that expose pause/unpause events, and only returns the metric when all required event topics have been indexed and backfilled.

- Improved x402 payment metadata for API clients. Payment-required responses now use the externally forwarded HTTPS URL when building the payment resource, include `USDC` in payment asset metadata, and can return the x402 flow for eligible endpoints without requiring a separate opt-in header.

- Improved Portal API-key activation. Newly created API keys now trigger an API-key cache refresh, reducing the delay between creating a key in the Portal and using it against the API.

**App**

- Updated protocol labels across vault discovery, dashboard positions, analytics breakdowns, opportunity pages, and protocol filters to use display names instead of raw protocol slugs.

- Improved protocol search and filtering so users can find protocols by either their canonical slug or their display name.
