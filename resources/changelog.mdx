...
- Expanded V2 vault and portfolio recommendation filters. `GET /v2/detailed-vaults`, `GET /v2/portfolio/best-vault/{userAddress}`, and `GET /v2/portfolio/best-deposit-options/{userAddress}` now expose additional filtering options including `maxApy`, `maxTvl`, `tags`, and `curators`. Portfolio recommendation endpoints now default toward app-featured vaults and exclude warning-flagged vaults unless requested.
...
- Updated app vault routes and internal links to use stable `vaultId` values instead of raw vault addresses. This improves support for non-address vault identifiers and keeps app links aligned with API identity fields.

- Stabilized the best-performing vaults cache warmup used by analytics views, reducing stale or missing chart data during cache refreshes.

- Added a shorter one-line MCP setup path on the hosted MCP landing page, making agent setup easier to copy directly into a terminal.
