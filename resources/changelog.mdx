...
- **[Reputation Score](/methodology/reputation-scores) now reflects active vault flags** — warnings and risk indicators directly apply severity-based penalties to vault scores: critical flags carry the largest deduction, warning flags moderate, and info-level flags minimal. Vaults with active warnings will show lower scores, providing a more accurate signal of current vault health. Scores are backwards compatible. Responses now include a `penaltyComponents` field detailing each active flag and its resolution status.

- **New `GET /v2/transactions/suffix/{userAddress}/{vaultId}` [endpoint](/api-reference/transactions)** — returns a raw hex suffix to append to transaction calldata, enabling on-chain attribution of deposits and withdrawals back to your API key.
