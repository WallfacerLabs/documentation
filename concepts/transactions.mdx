...
<CardGroup cols={2}>
  ...
  <Card title="Onchain referrals" icon="circle-dollar-sign">
    Append a builder code to transaction calldata via the suffix endpoint to register your integration as the onchain referrer and earn protocol rebates.
  </Card>
</CardGroup>

## Onchain referrals with the transaction suffix

The suffix endpoint returns a hex string that encodes your API key, the user address, and the vault as an onchain builder code. Appending it to transaction calldata registers your integration as the referrer at the protocol level — similar in spirit to ERC-8021 builder codes — and makes your integration eligible for referral rebates from protocols that support them.
...
Append the returned value to your transaction calldata **without** a `0x` prefix:

```js
const { calldata } = await getTransactionPayload(...)
const { suffix } = await getTransactionSuffix(userAddress, vaultId)
const calldataWithReferral = calldata + suffix
```

No intermediary contract is required. The referral is recorded onchain by the protocol itself.

<Note>
The suffix is specific to your API key, user address, and vault. Fetch it per-transaction — do not cache and reuse across different users or vaults.
</Note>
