...
| `score.penaltyComponents` | Active or decaying flag penalties applied to the composite score, with severity and resolution status for each. |
...
Use `flags` to surface active protocol notices in your UI, such as paused deposits, ongoing audits, or deprecation notices. Active flags also reduce the vault's Reputation Score — see [Flag penalties](/methodology/reputation-score#flag-penalties) for how severity and decay work.
