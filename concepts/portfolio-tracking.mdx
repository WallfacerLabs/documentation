...
description: "Track user positions across protocols, detect idle assets, surface personalized deposit options, and report PnL — all from one API."
sidebarTitle: "Portfolio Tracking"
---

The Vaults.fyi portfolio API gives you a unified view of a user's DeFi yield activity across all supported protocols and networks. Instead of querying each protocol separately and normalizing the results yourself, you get aggregated position data, transaction history, and deposit options for any wallet address in a single integration.

If you want a packaged position view instead of building the UI yourself, the [embeddable widget](/use-cases/embeddable-widget) can combine discovery, user positions, and transaction-ready flows in a branded Earn surface.
...
This is the same data that powers the personalized deposit options described below.

## Personalized deposit options
...
Deposit options are computed from the user's actual wallet holdings, so you surface only relevant opportunities. A user holding WETH on Arbitrum won't see USDC vaults on mainnet.

<Tip>
Use filters such as `minTvl` and `allowedProtocols` on the best-deposit-options endpoint to enforce your own quality bar before showing deposit options to users.
</Tip>
