...
<AccordionGroup>
  ...
  <Accordion title="Underlying asset reliability (12.5%)">
    Evaluates the stability and creditworthiness of the vault's deposit asset. For stablecoins and pegged assets (ETH/BTC derivatives), this component measures historical price stability relative to the peg, plus/minus 2% liquidity depth in onchain markets, and CEX and DEX trading volume. Assets with a consistent peg and deep liquidity contribute more positively.
  </Accordion>
...
When a flag is resolved, the penalty does not necessarily disappear immediately. Vaults.fyi can mark a flag with resolution details and reduce the effective penalty to reflect how much of the issue has been addressed. This lets a vault recover trust incrementally rather than resetting overnight.

The `score.penaltyComponents` field in [detailed-vaults](/api-reference/detailed-vaults) responses shows each active or decaying flag penalty, its severity, current penalty value, expiry, and optional `resolution` note. The `score.totalScorePenalty` field shows the total penalty currently applied to the composite Reputation Score. The same optional `resolution` note is also returned on `flags[]` entries.
