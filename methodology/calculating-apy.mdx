...
description: "Learn how Vaults.fyi derives APY from onchain share prices using trailing windows, TVL-weighted averages, and reward token accounting."
...
All APY values returned by Vaults.fyi are **net of fees**. Performance and management fees reflected in vault share price or yield accrual are already accounted for in the APY.

The fee breakdown is still exposed through the `fees` object on detailed-vault responses so integrators can display fees alongside yield. Do not subtract those fees from returned APY values again.

## Share price method

Every yield-bearing vault issues shares that represent a depositor's proportional stake. As the vault earns yield, the total value it holds grows relative to the number of shares outstanding, so each share becomes worth more. Vaults.fyi reads these values directly from onchain contracts hourly and derives APY from the change in share price over the chosen window.
